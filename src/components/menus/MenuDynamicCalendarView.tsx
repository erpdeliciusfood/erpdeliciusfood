...
import { format, startOfWeek, endOfWeek, startOfMonth, endOfMonth, eachDayOfInterval, isSameDay, parse } from "date-fns";
...
import { showError } from "@/utils/toast"; // FIX: Changed '=>' to 'from'
...
const MenuDynamicCalendarView: React.FC<MenuDynamicCalendarViewProps> = ({
  ...
  const modifiers: DayModifiers = {
    menus: menusInView?.map(menu => menu.menu_date ? parse(menu.menu_date, 'yyyy-MM-dd', new Date()) : undefined).filter(Boolean) as Date[],
    today: new Date(),
          ...
          {viewMode === 'week' ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-4">
              {daysToDisplay.map((day) => {
                const hasMenu = menusInView?.some((menu: Menu) => menu.menu_date && isSameDay(parse(menu.menu_date, 'yyyy-MM-dd', new Date()), day));
