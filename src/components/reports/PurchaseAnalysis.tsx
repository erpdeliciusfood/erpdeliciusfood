...
const PurchaseAnalysis: React.FC<PurchaseAnalysisProps> = ({ startDate, endDate }) => {
  ...
  const { insumosForPurchase, totalEstimatedPurchaseCost } = useMemo(() => {
    ...
    menus.forEach(menu => {
      ...
      if (isMenuInDateRange || (menu.event_type_id && menu.menu_date === null)) {
    ...
    return {
      insumosForPurchase: result.sort((a, b) => b.purchase_suggestion_rounded - a.purchase_suggestion_rounded),
      totalEstimatedPurchaseCost: parseFloat(overallEstimatedCost.toFixed(2)),
    };
  }, [menus, allInsumosData?.data, startDate, endDate, isLoading, isError]);
